...
enum {
  ...
  TK_NUM, TK_HEXNUM,TK_REG, TK_NEQ, TK_AND, DEREF,REV
};
...
static struct rule {
  ...
  {"\\$[\\$0-9a-zA-z]+|pc", TK_REG}, //registers: Can be more specific
  {"0(x|X)[0-9a-fA-F]+", TK_HEXNUM},  //Hex number: Must before number
  {"[0-9]+", TK_NUM}, //numbers
  {"-", '-'},   // minus
  {"\\*", '*'}, // multiply
  {"/", '/'},   // slide
      ...
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
        ...
        switch (rules[i].token_type) {
          ...
          case TK_HEXNUM:
            ...
            // store the matched string in the 'tokens'
